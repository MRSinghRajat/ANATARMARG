...
import 'sacred_text_audio_screen.dart';

class AllSacredTextsListenScreen extends ConsumerStatefulWidget {
  const AllSacredTextsListenScreen({super.key});

  @override
  ConsumerState<AllSacredTextsListenScreen> createState() => _AllSacredTextsListenScreenState();
}

class _AllSacredTextsListenScreenState extends ConsumerState<AllSacredTextsListenScreen> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final deities = allTexts.map((t) => t.deitySlug).where((d) => d != null && d.isNotEmpty).cast<String>().toSet().toList()..sort();

    var filtered = allTexts.toList();
    if (_selectedType != null) filtered = filtered.where((t) => t.type == _selectedType).toList();
    if (_selectedDeity != null) filtered = filtered.where((t) => t.deitySlug?.toLowerCase() == _selectedDeity!.toLowerCase()).toList();
    if (_searchQuery.isNotEmpty) {
      final q = _searchQuery.toLowerCase();
      filtered = filtered.where((t) => t.title.toLowerCase().contains(q) || (t.titleHindi?.toLowerCase().contains(q) ?? false) || (t.deitySlug?.toLowerCase().contains(q) ?? false)).toList();
    }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Sacred Texts Audio', style: GoogleFonts.crimsonPro(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white)),
      ),
      body: Column(
        children: [
          Padding(
            ...
            SizedBox(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  ...types.map((t) => _filterChip(SacredTextModel(id: '', slug: '', title: '', type: t).typeLabel, _selectedType == t, () => setState(() => _selectedType = _selectedType == t ? null : t))),
                ],
              ),
            ),
          if (deities.isNotEmpty)
            SizedBox(
              ...
              child: ListView(
                scrollDirection: Axis.horizontal,
                padding: const EdgeInsets.symmetric(horizontal: 16),
                children: deities.map((d) => _deityChip(d, _selectedDeity == d, () => setState(() => _selectedDeity = _selectedDeity == d ? null : d))).toList(),
              ),
            ),
          const SizedBox(height: 8),
          Expanded(
            child: textsAsync.isLoading
  ...
  Widget _buildCard(SacredTextModel text) {
    final gradient = _getDeityGradient(text.deitySlug);
    final hasImage = text.coverImageUrl != null && text.coverImageUrl!.isNotEmpty;
    return Material(
      color: Colors.transparent,
      child: InkWell(
        onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => SacredTextAudioScreen(sacredText: text))),
    ...
    return Padding(
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: active ? AppColors.matteGold : Colors.transparent,
            borderRadius: BorderRadius.circular(999),
            border: Border.all(color: active ? AppColors.matteGold : AppColors.matteGold.withValues(alpha: 0.2)),
          ),
    ...
    return Padding(
      ...
      child: GestureDetector(
        ...
        child: Container(
          padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
          decoration: BoxDecoration(
            color: active ? gradient.first : Colors.transparent,
            borderRadius: BorderRadius.circular(999),
            border: Border.all(color: gradient.first.withValues(alpha: active ? 1 : 0.3)),
          ),

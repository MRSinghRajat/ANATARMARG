...
function encode(value: unknown): string {
  return btoa(JSON.stringify(value)).replace(/=/g, "").replace(/\+/g, "-")
    .replace(/\//g, "_");
}
Deno.test("Apple identity verifies RSA signature and rejects wrong audience, expiration, and tampering", async () => {
  ...
  async function token(claims: Record<string, unknown>) {
    const input = `${encode({ alg: "RS256", kid: "test-key" })}.${
      encode(claims)
    }`;
  ...
  assert(
    await verifyAppleIdentityToken(good, "com.example.test", fetcher) ===
      "expected",
  );
  for (
    const bad of [
      await token({ ...claims, exp: now - 10 }),
      await token({ ...claims, aud: "another-app" }),
      `${good.split(".")[0]}.${encode({ ...claims, sub: "attacker" })}.${
        good.split(".")[2]
      }`,
    ]
  ) {
    let rejected = false;
    try {
      await verifyAppleIdentityToken(bad, "com.example.test", fetcher);
    } catch {
      rejected = true;
    }
    assert(rejected);

...
import type { DeletionOperations, DeletionJob } from './worker.ts';

export function deletionOperations(admin: SupabaseClient): DeletionOperations {
  return {
    async claim(userId) {
      const { data, error } = await admin.rpc('claim_account_deletion_job', { p_user_id: userId ?? null });
    ...
    async removeStorage(job) {
      ...
      for (let batch = 0; batch < 10; batch++) {
        const { data, error } = await admin.rpc('list_account_deletion_objects', {
          p_user_id: job.user_id, p_lease_token: job.lease_token,
        });
        if (error) throw new Error('storage_inventory_failed');
        if (!data?.length) return;
        const buckets = new Map<string, string[]>();
        for (const item of data as Array<{ bucket_id: string; name: string }>) {
          buckets.set(item.bucket_id, [...(buckets.get(item.bucket_id) ?? []), item.name]);
        }
    ...
    async finish(job, outcome) {
      const { data, error } = await admin.rpc('finish_account_deletion_job', {
        p_user_id: job.user_id, p_lease_token: job.lease_token,
        p_apple_revoked: outcome.appleRevoked, p_auth_removed: outcome.authRemoved,

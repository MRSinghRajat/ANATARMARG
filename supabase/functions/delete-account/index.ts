...
Deno.serve(async (req: Request) => {
  ...
  const { data: cleanup, error: cleanupError } = await userClient.rpc('delete_own_account_data');
  if (cleanupError) return jsonResponse({ ok: false, error: 'cleanup_failed' }, 500);
  if (cleanup?.ok !== true) {
    return jsonResponse({ ok: false, error: cleanup?.error ?? 'cleanup_failed' }, 409);
  }
  let status: 'complete' | 'pending' | 'busy' = 'pending';
  try { status = await processDeletionJob(deletionOperations(supabaseAdmin()), data.user.id); }
  catch { console.warn('delete-account: accepted job needs background retry'); }
  ...
  return jsonResponse({ ok: true, accepted: true, pending: status !== 'complete' }, status === 'complete' ? 200 : 202);
});
